...
@Component({
  selector: 'relationship-type-list',
  standalone: true,
  ...
  template: `
    <div class="list-panel">
      <div class="list-header">
        <h3 class="list-title"><mat-icon>share</mat-icon> Relationship Types</h3>
        ...
        <mat-selection-list [multiple]="false" class="item-list">
          @for (item of paged; track item.id) {
            <mat-list-option [value]="item" [selected]="selectedId === item.id" (click)="selected.emit(item)">
              ...
              <span matListItemLine class="meta-line">{{ item.id }}</span>
            </mat-list-option>
          }
          @if (filtered.length === 0) {
            <mat-list-item disabled><span matListItemTitle class="empty-text">No relationship types</span></mat-list-item>
          }
        </mat-selection-list>
...
export class RelationshipTypeListComponent {
  @Input() items: RelationshipType[] = [];
  @Input() loading                    = false;
  @Input() selectedId?: string;

  @Output() selected = new EventEmitter<RelationshipType>();
  ...
  get filtered(): RelationshipType[] {
    const q = this.query.trim().toLowerCase();
    return q ? this.items.filter((r) => r.name.toLowerCase().includes(q)) : this.items;
  }

  get paged(): RelationshipType[] {
